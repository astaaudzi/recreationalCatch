...
#select species and scale by the maximum value in each species separately
#(use cpue scaled to 1 and corresponding se in the input file).

myspecies  <- standart_cpue %>% filter(Species=="silverbream")

max(myspecies$cpue_new_scaled_to1, na.rm=TRUE) 

myspecies$stand.cpue.scaled <- myspecies$cpue_new_scaled_to1/max(myspecies$cpue_new_scaled_to1, na.rm=TRUE)   
plot(myspecies$cpue_new_scaled_to1, myspecies$stand.cpue.scaled)
## also scale se 
myspecies$Std_Error_stand_cpue.scaled <- myspecies$se_new_scaled*myspecies$stand.cpue.scaled/myspecies$cpue_new_scaled_to1
plot(myspecies$Std_Error_stand_cpue.scaled, myspecies$se_new_scaled)


#scale catches

#now we scale everything by one max of commercial and recreational catches (EJ: we have median of commercial catches, so we use "median" of recreational catches as well)
catchmax <- max(myspecies$commercial.catches, myspecies$recreat.catches.Anglers50, na.rm = T)  
catchmax

#catchmax <- max(myspecies$commercial.catches, myspecies$recreat.catches.Anglers90, na.rm = T) #EJ
...
plot1 <- ggplot(plot_data, aes(x=Year))+
  ...
  geom_text(x=2004, y= 1.05, label = '163 t', size=5, fontface=3, color="orange")+
  ...
  geom_text(x=2019, y= 0.1, label = '3.4 t', size=5, fontface=3, color= "red")+
     ylim(0,1.5) +
...
plot1 <- ggplot(plot_data, aes(x=Year))+
  ...
  geom_text(x=1997, y= 0.7, label = '20 t', size=5, fontface=3, color="orange")+
  #geom_text(x=2018, y= 0.5, label = 'Scientific CPUE (+error)', size=4, fontface=3, color = "steelblue")+
  geom_text(x=2019, y= 1.05, label = '32 t', size=5, fontface=3, color= "red")+
  geom_segment(aes(x = 2016, y = 	rec_mean[max(dim(plot_data))[1]], xend = 2021, yend = 	rec_mean[max(dim(plot_data))[1]]), color="red", lwd=1.5)+
  geom_segment(aes(x = 1990, y = 	rec_mean[max(dim(plot_data))[1]]/2, xend = 2016, yend = rec_mean[max(dim(plot_data))[1]]),lwd=1, color="red", linetype=2)+
  ylim(0,1.5)+
...
theme(
  ...
  geom_text(x=2007.5, y= 1, label = '36 t', size=5, fontface=3, color="orange")+
  #geom_text(x=2018, y= 0.5, label = 'Scientific CPUE (+error)', size=4, fontface=3, color = "steelblue")+
  geom_text(x=2019, y= 0.18, label = '4 t', size=5, fontface=3, color= "red")+
  geom_segment(aes(x = 2016, y = 	rec_mean[max(dim(plot_data))[1]], xend = 2021, yend = 	rec_mean[max(dim(plot_data))[1]]), color="red", lwd=1.5)+
  geom_segment(aes(x = 1990, y = 	rec_mean[max(dim(plot_data))[1]]/2, xend = 2016, yend = rec_mean[max(dim(plot_data))[1]]),lwd=1, color="red", linetype=2)+
  ylim(0,1.5)+
...
theme(
  ...
  geom_text(x=2006.5, y= 0.15, label = '1 t', size=5, fontface=3, color="orange")+
  #geom_text(x=2018, y= 0.5, label = 'Scientific CPUE (+error)', size=4, fontface=3, color = "steelblue")+
  geom_text(x=2019, y= 1.05, label = '9 t', size=5, fontface=3, color= "red")+
  geom_segment(aes(x = 2016, y = 	rec_mean[max(dim(plot_data))[1]], xend = 2021, yend = 	rec_mean[max(dim(plot_data))[1]]), color="red", lwd=1.5)+
  geom_segment(aes(x = 1990, y = 	rec_mean[max(dim(plot_data))[1]]/2, xend = 2016, yend = rec_mean[max(dim(plot_data))[1]]),lwd=1, color="red", linetype=2)+
  ylim(0,1.5)+
...
theme(
  ...
  geom_text(x=2003, y= 0.6, label = '10 t', size=5, fontface=3, color="orange")+
  #geom_text(x=2018, y= 0.5, label = 'Scientific CPUE (+error)', size=4, fontface=3, color = "steelblue")+
  geom_text(x=2019, y= 1.1, label = '19 t', size=5, fontface=3, color= "red")+
  geom_segment(aes(x = 2016, y = 	rec_mean[max(dim(plot_data))[1]], xend = 2021, yend = 	rec_mean[max(dim(plot_data))[1]]), color="red", lwd=1.5)+
  geom_segment(aes(x = 1990, y = 	rec_mean[max(dim(plot_data))[1]]/2, xend = 2016, yend = rec_mean[max(dim(plot_data))[1]]),lwd=1, color="red", linetype=2)+
  ylim(0,1.8)+
  annotate("rect", xmin=2016, xmax=2021, ymin=plot_data$rec_min[max(dim(plot_data))[1]], ymax=1.2,  alpha=0.2, fill='red')  #upper CI set as 1.2
